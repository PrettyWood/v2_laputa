...
def feed_db(app):
    app.mongo['test-sa_test']['front_config'].insert({'title': 'front config de prod poto!'})
    app.mongo['test-sa_test']['front_config-staging'].insert(
        {'title': 'front config de staging ma gueule!'})
    yield app
    app.mongo.drop_database('test-sa_test')
